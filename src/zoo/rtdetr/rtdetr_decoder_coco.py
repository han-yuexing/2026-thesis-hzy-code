...
class TransformerDecoder(nn.Module):
    def __init__(self, hidden_dim, decoder_layer, num_layers, reg_max, reg_scale, up, eval_idx=-1):
        ...
        self.pre_box = nn.Linear(hidden_dim, 4)
...
class RTDETRTransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        if self.training and self.aux_loss:
            ...
            out['aux_outputs'].extend(self._set_aux_loss2([enc_topk_logits][:-1], [enc_topk_bboxes][:-1], out_corners[:-1], out_corners[-1],
                                                          [enc_topk_logits][-1]))
            ...
            for i in range(1, len(out_logits)):
                ...
                out['enc_aux_outputs']= self._set_aux_loss2(
