...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses, alpha=0.2, gamma=2.0, eos_coef=1e-4, num_classes=80,reg_max=32):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        return losses
    def loss_local(self, outputs, targets, indices, num_boxes, T=4): ##2 4 5 6 10

        losses = {}
        
        if 'teacher_corners' in outputs:
            # print("teacher_corners")
            ...
            if not torch.equal(pred_corners, target_corners):
                ...
                if 'is_dn' not in outputs:
                    batch_scale = 2 / outputs['pred_boxes'].shape[0]  # Avoid the influence of batch size per GPU
    ...
    # def loss_distill(self, outputs, targets, indices, num_boxes):
        ...
        # distill_losses = []
        # if 'teacher_corners' in outputs:
        #     for i in range(1, len(outputs['pred_logits'])):
        #         teacher_logits = outputs['pred_logits'][i]
        #         teacher_corners = outputs['pred_corners'][i]

        #         student_logits = outputs['pred_logits'][i - 1]
        #         student_corners = outputs['pred_corners'][i - 1]

        #         if teacher_logits is not None:
        #             loss_logits = F.mse_loss(student_logits, teacher_logits.detach(), reduction='none')
        #             loss_logits = loss_logits.sum() / num_boxes
        #         else:
        #             loss_logits = 0.0

        #         if teacher_corners is not None:
        #             loss_corners = F.mse_loss(student_corners, teacher_corners.detach(), reduction='none')
        #             loss_corners = loss_corners.sum() / num_boxes
        #         else:
        #             loss_corners = 0.0

        #         distill_losses.append(loss_logits + loss_corners)

        #     losses['loss_ddl'] = sum(distill_losses) / len(outputs['pred_logits'])
        # return losses
    ...
    def forward(self, outputs, targets):
        ...
        if 'enc_aux_outputs' in outputs:
            for i, enc_aux_outputs in enumerate(outputs['enc_aux_outputs']):
                # print(aux_outputs.keys())
                indices = self.matcher(enc_aux_outputs, targets)
                for loss in self.losses:
                    if loss == 'masks':
                        # Intermediate masks losses are too costly to compute, we ignore them.
                        continue
                    ...
                    l_dict = self.get_loss(loss, enc_aux_outputs, targets, indices, num_boxes, **kwargs)
                    l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
                    l_dict = {k + f'_enc_aux_{i}': v for k, v in l_dict.items()}
                    losses.update(l_dict)
